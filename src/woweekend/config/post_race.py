...
class PostRaceConfig:
    ...
    def parse(cls, value: Mapping[str, Any]) -> "PostRaceConfig":
        ...
        reject_unknown(data, {"season", "round_number", "race_start", "pre_race_run_id", "qualifying_performance", "race_performance", "data_root", "report"}, path="config")
        ...
        return cls(season, round_number, race_start, data.get("pre_race_run_id"), AnalysisOverrides.parse(data.get("qualifying_performance"), path="qualifying_performance"), AnalysisOverrides.parse(data.get("race_performance"), path="race_performance"), data.get("data_root"), ReportConfig.parse(data.get("report")))

    @staticmethod
    def template() -> dict[str, Any]:
        return {"_help": {"race_start": "Automatically resolved from FastF1 event/session metadata. Leave null normally; set an ISO-8601 timestamp only to correct missing/incorrect metadata.", "pre_race_run_id": "Identifies the saved Race Preparation run treated as the genuine pre-race prediction. Leave null to select the final usable run before race start; set a RUN_ID only to force a specific historical run.", "post_strategy_status": "Retro Green Optimum is calculated automatically. Event-Aware Hindsight Optimum may be unavailable until its deterministic SC/VSC API is extracted from woPlanner."}, "race_start": None, "pre_race_run_id": None, "qualifying_performance": {"_help": {"overrides": "Passed only to parameters supported by the existing woStrategy qualifying performance API."}, "overrides": {}}, "race_performance": {"_help": {"overrides": "Passed only to parameters supported by the stable woStrategy race-review adapter."}, "overrides": {}}, "data_root": None, "report": {"language": "zh-CN", "template": "engineering"}}
